...
overlay = Overlay('/home/xilinx/pynq/overlays/EventsToImage/EventsToImage.bit')


# Constantes
RES = 128           # Resolución del sensor
MAX_EVENTS = 20000  # Máximo de eventos que caben en el input buffer
...
input_buffer = allocate(shape=(MAX_EVENTS*3,), dtype=np.uint32)
...
while True:
    try:
        ...
        [events, num_events] = dvslib.get_image('raw')

        # Pasa el IP el número de eventos del paquete
        overlay.eventsToImage_0.register_map.num_events = num_events

        # Elimina la columna de los timestamp y una última columna residual
        d = np.delete(events, 0, axis=1)
        d = np.delete(d, 3, axis=1)

        # Transforma la matriz en un array lineal y lo introduce en el buffer de entrada
        input_buffer[0:len(d)*3] = np.reshape(d, len(d) * 3)
