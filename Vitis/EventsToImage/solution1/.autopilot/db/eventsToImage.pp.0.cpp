# 1 "EventsToImage/eventsToImage.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 395 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "EventsToImage/eventsToImage.cpp" 2
# 1 "EventsToImage/eventsToImage.h" 1
...
# 2 "EventsToImage/eventsToImage.h" 2
# 1 "C:/Xilinx/Vitis_HLS/2020.2/common/technology/autopilot\\ap_int.h" 1
# 3 "EventsToImage/eventsToImage.h" 2
...
__attribute__((sdx_kernel("eventsToImage", 0))) void eventsToImage(stream_type &input, stream_type &output, ap_uint<32> num_events);
# 2 "EventsToImage/eventsToImage.cpp" 2

__attribute__((sdx_kernel("eventsToImage", 0))) void eventsToImage(stream_type &input, stream_type &output, ap_uint<32> num_events) {
#pragma HLS TOP name=eventsToImage
# 3 "EventsToImage/eventsToImage.cpp"
...
#pragma HLS INTERFACE axis port=output
 ...
 type static aux[128*128];
 int static img[128*128];




 VITIS_LOOP_17_1: for(int i = 0; i < 128*128; i++)
 {
#pragma HLS PIPELINE
 ...
 VITIS_LOOP_29_2: for(int i = 0; i < 20000; i++)
 {
#pragma HLS PIPELINE
 ...
 type x = input.read();
  ...
  if(i < num_events)
   img[(int)y.data * 128 + (int)x.data] = (int)p.data ? 255 : 0;



  if(i == 20000 - 1){
   aux[128*128 -1].keep = p.keep;
   aux[128*128 -1].strb = p.strb;
   aux[128*128 -1].user = p.user;
   aux[128*128 -1].last = p.last;
   aux[128*128 -1].id = p.id;
   aux[128*128 -1].dest = p.dest;
  }
 ...
 VITIS_LOOP_74_3: for(int i = 0; i < 128*128; i++)
 {
#pragma HLS PIPELINE
...
# 211 "EventsToImage/eventsToImage.cpp"
}
