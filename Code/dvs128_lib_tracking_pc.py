'''
DVS128 Tracking using CAMShift for Ubuntu using DVSAbsLib library.
...
dvslib = dal.DVSAbsLib(max_packet_interval=12000, noise_filter=True)
dvslib.init()
dvslib.device_info()
...
def time_stats(resolution='ms'):
    ...
    tiempos = {'Get_image':mean_get_image, 'Umbral':mean_umbral, 'Dilatación y apertura':mean_dilata, 'Tracking':mean_tracking, 'CAMShift':mean_camshift, 'Show Img':mean_hdmi}
...
while True:
    try:
        ...
        # Muestra la imagen final por pantalla con escalado x5
        dvslib.show_img(res, scale=5)
            
        # Corta la reproducción del video pulsando 'q'
